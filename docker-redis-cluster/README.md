...
If the flag `-e "SENTINEL=true"` is passed there are 3 Sentinel nodes running on ports 15000 to 15002 matching cluster's master instances.
...
To build a different redis version use the `--build-arg` argument.
    ...
    docker-compose build --build-arg "redis_version=4.0.14" redis-cluster
